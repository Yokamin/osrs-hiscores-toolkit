import pytest
from src.hiscores_parser import get_skill_data, get_activity_data
...
def test_get_skill_data_found(mock_player_data):
...
def test_get_skill_data_not_found(mock_player_data):
    """Tests get_skill_data for a non-existent skill."""
    skill = get_skill_data(mock_player_data, "Woodcutting")
    assert skill is None

def test_get_skill_data_empty_player_data(mock_player_data):
...
def test_get_skill_data_no_skills_key(mock_player_data):
...
def test_get_activity_data_found(mock_player_data):
